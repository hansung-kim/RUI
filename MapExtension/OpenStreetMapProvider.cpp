...
OpenStreetMapProvider::OpenStreetMapProvider(int type, const AnsiString& subDir)
    : mapType(type), mapSubDir(subDir) {}

void OpenStreetMapProvider::Initialize(bool loadFromInternet, const AnsiString& baseDir) {
    ...
    tileManager = new TileManager(storage, false);
}

TileManager* OpenStreetMapProvider::GetTileManager() {
    return tileManager;
}

GoogleLayer* OpenStreetMapProvider::CreateLayer() {
